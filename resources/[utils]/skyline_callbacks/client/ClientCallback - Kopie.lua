...
function TriggerWithTimeout(name, timeout, ...)
	...
	-- send data to server
	TriggerServerEvent("SKYLINE:sc", name, requestId, { ... })
...
RegisterNetEvent("SKYLINE:cc")
AddEventHandler("SKYLINE:cc", function(name, requestId, data)
	local requestName = name .. tostring(requestId)
	...
	if (callbacks[name] ~= nil) then
		...
		TriggerServerEvent("SKYLINE:ccResponse", requestName, result)
	else
		...
		TriggerServerEvent("SKYLINE:ccDoesNotExist", requestName, name)
	end
end)

RegisterNetEvent("SKYLINE:scResponse")
AddEventHandler("SKYLINE:scResponse", function(requestName, data)
...
RegisterNetEvent("SKYLINE:scDoesNotExist")
AddEventHandler("SKYLINE:scDoesNotExist", function(requestName, name)
	if (callbackResponses[requestName] ~= nil) then
		callbackResponses[requestName] = "ERROR"
