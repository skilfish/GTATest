...
function TriggerWithTimeout(name, playerId, timeout, ...)
	...
	-- send data to client
	TriggerClientEvent("SKYLINE:cc", playerId, name, requestId, { ... })
...
RegisterNetEvent("SKYLINE:sc")
AddEventHandler("SKYLINE:sc", function(name, requestId, data)
	local src = source
	...
	if (callbacks[name] ~= nil) then
		...
		TriggerClientEvent("SKYLINE:scResponse", src, requestName, result)
	else
		-- callback does not exist
		print(
			"^1[ERROR] ServerCallback \"" .. name .. "\" does not exist!^0\n" .. 
			"    Potential solutions:\n" .. 
			"    - \"SKYLINEmi_callbacks\" needs to be started before the script that is using this export!\n" .. 
			"    - Make sure that there is no typo in the Register or Trigger function!"
		)

		TriggerClientEvent("SKYLINE:scDoesNotExist", src, requestName, name)
	end
end)

RegisterNetEvent("SKYLINE:ccResponse")
AddEventHandler("SKYLINE:ccResponse", function(requestName, data)
...
RegisterNetEvent("SKYLINE:ccDoesNotExist")
AddEventHandler("SKYLINE:ccDoesNotExist", function(requestName, name)
	if (callbackResponses[requestName] ~= nil) then
		callbackResponses[requestName] = "ERROR"
