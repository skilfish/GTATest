if Config.Player then 
    if Config.PlayerWeight then 
        SKYLINE.RegisterCommand('openinv', 'admin', function(xPlayer, args, showError)
            local target = SKYLINE.GetPlayerFromId(args.id)
            if not target then 
        ...
        TriggerEvent('es:addGroupCommand', 'openinv', 'admin', function(source, args, user)
            local xPlayer = SKYLINE.GetPlayerFromId(args.id)
