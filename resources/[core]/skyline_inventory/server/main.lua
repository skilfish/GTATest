...
SKYLINE.RegisterServerCallback("inventory:getInventory", function(source, cb)
...
SKYLINE.RegisterServerCallback("inventory:getOtherInventory", function(source, cb, otherInv)
    local xPlayer = SKYLINE.GetPlayerFromId(source)
    if not xPlayer then return end
        ...
        for k,v in pairs(otherInv.shopItems) do
            if v.type == 'weapon' then
                table.insert(items, {
                    type = 'item_weapon',
                    name = v.name,
                    label = SKYLINE.GetWeaponLabel(v.name),
                    count = v.price
                })
    ...
    if otherInv.type == 'player' then
        local target = SKYLINE.GetPlayerFromId(otherInv.id)
        ...
        if item.type == 'item_account' then
            if item.name == 'cash' then
                if target.getMoney() >= count then
                    ...
                    TriggerClientEvent('inventory:notify', src, 'success', ('%s <b>%s</b> moved to <b>%s</b>!'):format(count, item.label, 'Deine Tasche'))
        ...
        elseif item.type == 'item_weapon' then
            if target.hasWeapon(item.name) then
                if not xPlayer.hasWeapon(item.name) then
                    ...
                    xPlayer.addWeapon(item.name, item.count)
                    TriggerClientEvent('inventory:notify', src, 'success', ('<b>%s</b> moved to <b>%s</b>!'):format(item.label, 'Deine Tasche'))
...
AddEventHandler('skyline:playerDropped', function(source)
