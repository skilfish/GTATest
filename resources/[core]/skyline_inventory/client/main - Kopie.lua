...
RegisterNetEvent('inventory:notify', function(type, msg)
    TriggerEvent("skyline_notify:Alert", "TASCHE" , msg , 4000 , type)
end)
...
RegisterNUICallback('UseItem', function(data)
    ...
    TriggerServerEvent('skyline:useItem', data.item.name)
    Wait(150)
        ...
        if data.item.type == 'item_account' then
            if data.item.name == 'cash' then
                if not Config.PlayerWeight then
                    TriggerServerEvent("skyline:giveInventoryItem", data.src, 'item_money', 'cash', tonumber(data.count)) 
                else 
                    TriggerServerEvent("skyline:giveInventoryItem", data.src, data.item.type, 'money', tonumber(data.count)) 
                end
            else 
                TriggerServerEvent("skyline:giveInventoryItem", data.src, data.item.type, data.item.name, tonumber(data.count)) 
            end
        else
            TriggerServerEvent("skyline:giveInventoryItem", data.src, data.item.type, data.item.name, tonumber(data.count)) 
        end
...
RegisterNUICallback('RemoveItem', function(data, cb)
    ...
    for k,v in pairs(Drops) do
        ...
        if distance < 2.0 then
            TriggerServerEvent('inventory:moveItemToOther', data.item, tonumber(data.count), {id = k, type = 'drop', title = '🗑️ Auf dem Boden - ' .. k})
            Wait(50)
            cb(200)
            return
        end
...
RegisterCommand('openInventory', function()
    ...
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        if Config.Glovebox then
            ...
            local plate = SKYLINE.Math.Trim(GetVehicleNumberPlateText(vehicle))

            if Config.GloveboxSave then
                SKYLINE.TriggerServerCallback("inventory:isVehicleOwned", function(owned)
                    OpenInventory({
                        id = plate, 
                        type = 'glovebox', 
        ...
        for k,v in pairs(Drops) do
            local distance = #(v.coords - GetEntityCoords(PlayerPedId()))
            if distance < 2.0 then
                OpenInventory({id = k, type = 'drop', title = '🗑️ Auf dem Boden - ' .. k, coords = v.coords, weight = false, timeout = 1000})
                return
            end
        end 
        OpenInventory()
