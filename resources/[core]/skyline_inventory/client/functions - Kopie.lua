function OpenInventory(otherInv)
    ...
    SendNUIMessage({
        action = 'open',
        inventory = {type = 'main', title = 'Deine <b>Tasche</b>', weight = Config.PlayerWeight},
...
function GetInventory(otherInv, refresh)
    if otherInv then
        SKYLINE.TriggerServerCallback("inventory:getOtherInventory", function(items, weight)
        ...
        SKYLINE.TriggerServerCallback("inventory:getInventory", function(items, weight, hotbar)
            SendNUIMessage({
                action = 'setItems',
                items = items,
                weight = weight,
...
function CloseInventory()
    ...
    if not IsPedInAnyVehicle(PlayerPedId()) then
        ...
        SKYLINE.Streaming.RequestAnimDict('pickup_object', function()
...
function GetPlayersInArea()
    ...
    for k,v in pairs(SKYLINE.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 5.0)) do 
    ...
    SKYLINE.TriggerServerCallback("inventory:getInventory", function(items, weight, hotbar)
        ...
        if item then
            ...
            if item.type == 'item_standard' then
                if data.use then
                    TriggerServerEvent('skyline:useItem', item.name) 
                end
            elseif item.type == 'item_weapon' then
                if not weaponLock then
                    ...
                    if not weaponEquiped or weaponEquiped.name ~= item.name then
                        weaponEquiped = item
                        SKYLINE.Streaming.RequestAnimDict('reaction@intimidation@1h', function()
