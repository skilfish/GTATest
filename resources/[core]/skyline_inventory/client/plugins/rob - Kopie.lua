if Config.Rob then
    ...
    RegisterCommand('rob', function()
        ...
        if player ~= -1 and distance <= 3.0 then
            ...
            if DoesEntityExist(ped) then
                SKYLINE.Streaming.RequestAnimDict("random@mugging5", function()
                ...
                while true do 
                    Wait(1)
                    player, distance = SKYLINE.Game.GetClosestPlayer()
    ...
    RegisterCommand('handsup', function()
        if not handsup then
            handsup = true
            SKYLINE.Streaming.RequestAnimDict("missminuteman_1ig_2", function()
