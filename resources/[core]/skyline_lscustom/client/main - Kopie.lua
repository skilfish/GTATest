...
AddEventHandler('skyline:playerLoaded', function(xPlayer)
	...
	--ESX.TriggerServerCallback('skyline_lscustom:getVehiclesPrices', function(vehicles)
	--	Vehicles = vehicles
	--end)
end)
...
RegisterNetEvent('skyline_lscustom:installMod')
AddEventHandler('skyline_lscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('skyline_lscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('skyline_lscustom:cancelInstallMod')
AddEventHandler('skyline_lscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('skyline_lscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('skyline_lscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('skyline_lscustom:cancelInstallMod')
...
function GetAction(data)
	...
	ESX.TriggerServerCallback('skyline_lscustom:isMechanic', function(mechanic)

        if mechanic then
			OpenLSMenu(elements, menuName, menuTitle, parent)
        else 
          TriggerEvent("skyline_notify:Alert", "#ff0000" , "Werkstatt" , "Du bist kein Mechaniker")
        end
				...
				if (PlayerData.job and PlayerData.job.name == 'mechanic') or not Config.IsMechanicJobOnly then
					...
					ESX.TriggerServerCallback('skyline_lscustom:isMechanic', function(mechanic)
						...
						if mechanic then
							...
							TriggerEvent("skyline_notify:Alert", "WERKSTATT" , "Dieses Fahrzeug gehört dir nicht!" , 3000 , "errors")
						end
