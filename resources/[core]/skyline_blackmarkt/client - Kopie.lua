...
mainMenu = NativeUI.CreateMenu("Schwarzmarkt","~b~Was willst du kaufen?", 5, 100,"shopui_title_gr_gunmod", "shopui_title_gr_gunmod",nil,255,255,255,230)
...
function AddShopsIllegalMenu()
    ...
    mainMenu.OnItemSelect = function(_,item,Index)
        ESX.TriggerServerCallback('dukleinernuttensohn:buyWeapon', function(bought)
            if bought then
...
function DisplayBoughtScaleform(weaponName, price)
	...
	PushScaleformMovieMethodParameterString("Du hast ".. ESX.GetWeaponLabel(weaponName).." gekauft für ".. price .. "$")
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 2.5 then
            isNearShop = true
            ESX.ShowHelpNotification("~INPUT_TALK~ um mit dem ~r~Verkäufer ~w~zu handeln")
