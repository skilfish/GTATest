    ...
    ESX.TriggerServerCallback('fleecaheist:server:checkPoliceCount', function(status)
        if status then
            ESX.TriggerServerCallback('fleecaheist:server:checkTime', function(time)
                if time then
                    TriggerEvent("skyline_notify:Alert", "RAUB" , "Bankraub gestartet!" , 2000 , "success")
                    local ped = PlayerPedId()
                    local pedCo = GetEntityCoords(ped)
...
function Outside(index)
    ...
    TriggerServerEvent('fleecaheist:server:resetHeist', currentBank)
    loadModel('baller')
...
function ShowNotification(msg)
    TriggerEvent("skyline_notify:Alert", "RAUB" , msg , 3000 , "long")
end
