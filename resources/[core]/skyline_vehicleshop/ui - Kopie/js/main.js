...
$(window).ready(function() {
    window.addEventListener('message', (event) => {
        ...
        if (data.action == "load"){
            ...
            intv = setInterval(() => {
                curTest = curTest - 1;
                $("#delivery-time").html("Probefahrt<br/>Du hast noch " + curTest + " Sekunden");
            }, 1000);
            ...
            if(intv) { clearInterval(intv);  }
            curTest = 15;
            $("#delivery-time").html("LIEFERUNG:<br/>JETZT");
        }
    });
});
...
function init(data) {
   ...
   $("#premium-info").html(`LADEN:<br/>${garage.name}`);
...
function loadCat(x) {
   ...
   $.each(garage.Vehicles[carCategories[x]], function (key, val) {
	   ...
	   if(key == 0) {
		  active = `active`;
		  $.post("https://skyline_vehicleshop/showCar", JSON.stringify({ name: val.name }), function(x){});
          setPrice(val.price);
          $("#vehicle-name").html(val.name);
    

          $("#tank-lt").html(val.fuel + " LITER" );
...
$(document).on('click', '.vehicle-list-name', function(e){
    ...
    $.post("https://skyline_vehicleshop/showCar", JSON.stringify({ name: $(this).data("name") }), function(x){});
    
    $("#vehicle-name").html($(this).data("name"));

    $("#tank-lt").html($(this).data("fuel")  + " LITER" );
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: 
            $("#wrap").hide();
            $("body").hide();
			$.post("https://skyline_vehicleshop/close", JSON.stringify({}), function(x){});
            break;
    }
});
...
function testdrive(){
            $("#wrap").hide();
            $("body").hide();
			$.post("https://skyline_vehicleshop/testdrive", JSON.stringify({}), function(x){});
}
...
async function moveRight(){
    ...
    while (startMovingRight){
        $.post('https://skyline_vehicleshop/moveright');
        await sleep(5);
    }
...
async function moveLeft(){
    ...
    while (startMovingLeft){
        $.post('https://skyline_vehicleshop/moveleft');
        await sleep(5);
    }
...
$(document).on('click', '.choose-color-circle', function(e){
    ...
    $.post("https://skyline_vehicleshop/setcolour", JSON.stringify({rgb: rgb}), function(x){});
});
...
function plakaonay() { 
    ...
    $.post("https://skyline_vehicleshop/checkPlatePrice", JSON.stringify({plate: plate}), function(x){
...
function buy() {
   ...
   $.post("https://skyline_vehicleshop/buy", JSON.stringify({blackMoney: blackMoney}), function(x){ 
        if(x == true) {  
            $("#wrap").hide();
            $("body").hide();
			//$.post("https://skyline_vehicleshop/close", JSON.stringify({}), function(x){});
            ...
            $("#btn-buy-black-money").animate({ color: "white", backgroundColor: "rgb( 255, 0, 0 )" });
            setTimeout(() => {
                $("#btn-buy-money").animate({ color: "white",  backgroundColor: "rgb( 115,200,50 )" });
                $("#btn-buy-black-money").animate({ color: "white", backgroundColor: "rgb( 115,200,50 )" });
            }, 1000);
        }
    }); 
}
...
$(document).ready(function() {
    $('.colorpicker').minimalColorpicker({
        ...
        onUpdateColor: function(e, color) {
            $.post("https://skyline_vehicleshop/setcolour", JSON.stringify({rgb: color.rgb}), function(x){});
        }
    });
});
