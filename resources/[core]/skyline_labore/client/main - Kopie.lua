  ...
  while true do
    ...
    if not dead then
      ...
      if Config.ShowDebugText and area then
        ...
        if closest == "PBLUFF" then 
          testText.text = "Zone: ".."Meth-Küche".."\nBesitzer: "..area.control:sub(1,1):upper()..area.control:sub(2).."\nEinfluss: "..math.floor(area.influence).."%"
      ...
      for handle,blip in pairs(v.blips) do
        if Config.DisplayZoneForAll or PlayerInGang() then
          if blip.color ~= BlipColors[v.control] or blip.alpha ~= math.floor(v.influence) then 
            ...
            mLibs:SetBlip(handle,"shortRange",true)
...
RegisterCommand("test_1" , function()
  print(GetNameOfZone(GetEntityCoords(GetPlayerPed(-1))))
end)
