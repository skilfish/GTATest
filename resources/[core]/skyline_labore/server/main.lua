...
Init = function()
    ...
    if doCont then
        ...
        {["@territories"] = "territories"},
            function(data)
                if data then
                    MySQL.Async.fetchAll("SELECT * FROM territories", {} , function(retData)
                            if retData then
                                for k, v in pairs(retData) do
                                    if v then
                                        local zone = v["zone"]
                                        local control = v["control"]
                                        local influence = v["influence"]

                                        print("Zone: " .. zone)
                                        print("Control: " .. control)
                                        print("Influence: " .. influence)

                                        Territories[zone].control = control
                                        Territories[zone].influence = influence
                            ...
                            for k, v in pairs(Territories) do
                                if v and type(v) == "table" and v.control and v.influence then
                                    ...
                                    MySQL.Async.execute(
                                        ...
                                        {["@control"] = v.control, ["@influence"] = v.influence, ["@zone"] = v.zone}
                                    )
                                end
                            end
                        end
