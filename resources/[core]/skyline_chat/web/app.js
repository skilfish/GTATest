window.APP = {
	...
	mounted() {
		post('https://skyline_chat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if(this.message !== '') {
				post('https://skyline_chat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('https://skyline_chat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
